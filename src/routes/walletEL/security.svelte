<script>

</script>

<div class="sc-dkPtRN jScFby scroll-view sc-eCFVrV hJfzJj">
    <div class="sc-jaskfy faaQQX">
        <div class="google-step-summary-top">Download and install
            <a target="_blank" href="https://support.google.com/accounts/answer/1066447?hl=en&amp;rd=1">Google Authenticator</a>.
            Enable Two-factor Authentication to protect your account from unauthorized access.</div>
        <div class="google-step-summary-top two">Scan the QR code with your Google Authenticator App or enter the secret key manually.</div>
        <div class="qrcode-warp">
            <img src="/api/user/google/2-step-auth/key/qrcode/320/?t=1697747350506" alt="qrcode">
        </div><div class="copy-input">
            <div class="sc-ezbkAF gcQjQT input sc-fbyfCU fWAvBM">
                <div class="input-label">Your secret key</div>
                <div class="input-control">
                    <input type="text" readonly="" value="oipnnq5gw4rva2rlydb7lcdyk4blwsll">
                    <button class="sc-iqseJM cBmlor button button-normal copy-button">
                        <div class="button-inner">
                            <svg xmlns:xlink="http://www.w3.org/1999/xlink" class="sc-gsDKAQ hxODWG icon">
                                <use xlink:href="#icon_Copy"></use>
                            </svg>
                        </div>
                    </button>
                </div>
            </div>
        </div>
        <p class="twofa-alert">Write down this code, never reveal it to others. You can use it to regain access to your account if there is no access to the authenticator.</p>
        <div class="codes">
            <p>Verification code</p>
            <div class="sc-cvZCdy hooaaA">
                <div class="google-input">
                    <input autocomplete="new-password" type="password" value="">
                    <ul class="">
                        <li class="active"></li>
                        <li class=""></li>
                        <li class=""></li>
                        <li class=""></li>
                        <li class=""></li>
                        <li class=""></li>
                    </ul>
                </div>
            </div>
        </div>
        <div class="sc-ezbkAF gcQjQT input sc-bYoBSM ixxYMF">
            <div class="input-control">
                <input type="password" autocomplete="new-password" placeholder="Login Password" value="">
                <svg xmlns:xlink="http://www.w3.org/1999/xlink" class="sc-gsDKAQ hxODWG icon">
                    <use xlink:href="#icon_View"></use>
                </svg>
            </div>
        </div>
        <button class="sc-iqseJM sc-egiyK cBmlor fnKcEH button button-normal">
            <div class="button-inner">Enable</div>
        </button>
    </div>
</div>

<style>
.hJfzJj{
    border-radius: 20px;
    background-color: var(--card-bg-5);
    padding: 1.25rem 1.25rem 0px;
}
.sc-dkPtRN{
    flex: 1 1 0%;
}
.hJfzJj > div {
    padding-bottom: 2rem;
    margin-top: 0.5rem;
}
.hJfzJj .google-step-summary-top {
    padding: 1rem 1.25rem;
    border-radius: 0.625rem;
    border: 1px solid var(--border-1);
    background-color: var(--sidebar-card-bg);
}
.faaQQX .google-step-summary-top {
    font-size: 0.75rem;
    line-height: 1.25rem;
}
.faaQQX .google-step-summary-top a {
    text-decoration: underline;
    margin-left: 0.25rem;
}
.hJfzJj .google-step-summary-top.two {
    background-color: transparent;
    border: none;
    padding: 0px 1.25rem;
    margin-top: 0.75rem;
}
.faaQQX .google-step-summary-top.two {
    margin-top: 0.75rem;
}
.hJfzJj .google-step-summary-top {
    padding: 1rem 1.25rem;
    border-radius: 0.625rem;
    border: 1px solid var(--border-1);
    background-color: var(--sidebar-card-bg);
}
.faaQQX .google-step-summary-top {
    font-size: 0.75rem;
    line-height: 1.25rem;
}
.faaQQX .google-step-summary-top {
    font-size: 0.75rem;
    line-height: 1.25rem;
}
.hJfzJj .qrcode-warp {
    margin-top: 0.75rem;
    background: rgb(245, 246, 247);
    width: 8.625rem;
    height: 8.625rem;
}
.faaQQX .qrcode-warp {
    padding: 0.3125rem;
    background: rgb(255, 255, 255);
    border-radius: 0.75rem;
    font-size: 0px;
    width: 8.125rem;
    margin: 1.25rem auto 0px;
    display: flex;
    -webkit-box-align: center;
    align-items: center;
    -webkit-box-pack: center;
    justify-content: center;
}
.hJfzJj .qrcode-warp img {
    width: 8rem;
    height: 8rem;
}
.hJfzJj .copy-input {
    margin-top: 0.75rem;
}
.hJfzJj .input {
    margin-top: 1.5rem;
}
.hJfzJj .copy-input .input-label {
    margin-left: 0.625rem;
}
.gcQjQT .input-label {
    display: flex;
    -webkit-box-align: center;
    align-items: center;
    line-height: 1em;
    height: 1.25rem;
    margin: 0px 1.125rem 0.375rem;
color: var(--text-6);}
.hJfzJj .input .input-control {
    height: 3.5rem;
}
.gcQjQT .input-control {
    position: relative;
    display: flex;
    -webkit-box-align: center;
    align-items: center;
    border: 1px solid var(--border-1);
    background-color: var(--sidebar-card-bg);
    opacity: 1;
    height: 2.75rem;
    border-radius: 1.5rem;
    padding: 0px 1.375rem;
}
.hJfzJj .copy-input .input-control input[readonly] {
    opacity: 1;
}
.gcQjQT .input-control input {
    flex: 1 1 0%;
    width: 100%;
    height: 100%;
    min-width: 1rem;
    padding: 0px;
    border: none;
    background-color: transparent;
    color: var(--text-5);
}
.faaQQX .copy-input .input-control .copy-button {
    width: 2.5rem;
    height: 2.5rem;
    margin-top: 0.125rem;
}
.hJfzJj .twofa-alert {
    margin: 1.125rem 0px 0px;
    padding: 0px 1.25rem 2.25rem;
    border-bottom: 1px solid rgba(176, 179, 191, 0.1);
}
.faaQQX .twofa-alert {
    font-size: 0.75rem;
    line-height: 1.25rem;
    margin-top: 0.75rem;
}
.hJfzJj .codes {
    margin-top: 1.5rem;
}
.hJfzJj .codes p {
    margin: 0px;
}
.faaQQX .codes p {
    height: 1.25rem;
    line-height: 1.25rem;
    margin-top: 1rem;
    text-align: center;
}
.hooaaA {
    width: 100%;
    text-align: center;
}
.hJfzJj .codes .google-input {
    margin-top: 0.875rem;
}
.hooaaA .google-input {
    position: relative;
    display: inline-block;
    width: auto;
}
.hooaaA .google-input > input {
    box-sizing: border-box;
    display: block;
    position: absolute;
    top: 0px;
    left: -9999px;
    width: 99999px;
    height: 100%;
    border: none;
    background-color: transparent;
    color: transparent;
    caret-color: transparent;
}
.hooaaA .google-input > ul {
    padding: 0px;
    display: flex;
    -webkit-box-pack: justify;
    justify-content: space-between;
    margin: 0px auto;
}
.hooaaA .google-input > ul li {
    box-sizing: border-box;
    width: 40px;
    height: 40px;
    margin-right: 0.375rem;
    border-radius: 0.625rem;
    font-size: 1rem;
    background-color: rgba(30, 32, 36, 0.6);
    border: 1px solid transparent;
    font-weight: bold;
    display: flex;
    -webkit-box-align: center;
    align-items: center;
    -webkit-box-pack: center;
    justify-content: center;
}
.hJfzJj .codes .google-input ul li {
    width: 3.5rem;
    height: 3.5rem;
    border-radius: 1.25rem;
    margin-right: 0.625rem;
    border: 1px solid var(--border-1);
    background-color: var(--sidebar-card-bg);
    color: var(--text-5);
}
li {
    margin: 0.5em 0;
}
.hJfzJj .input {
    margin-top: 1.5rem;
}
.hJfzJj .input .input-control {
    height: 3.5rem;
}
.gcQjQT .input-control {
    position: relative;
    display: flex;
    -webkit-box-align: center;
    align-items: center;
    border: 1px solid var(--border-1);
    background-color: var(--sidebar-card-bg);
    opacity: 1;
    height: 2.75rem;
    border-radius: 1.5rem;
    padding: 0px 1.375rem;
}
.hJfzJj > div > button {
    margin-top: 2rem;
}
.faaQQX > button {
    width: 11.25rem;
    height: 3rem;
    margin: 1.25rem auto 0px;
}


</style>
