    ...
    <div class="s866wzp transaction-wrap">
        <div class="s80qwti transaction-tabs">
            <div class="ui-select is-open">
                ...
                <div class="select-options-wrap" style={`top: 100%; opacity: 1; transform: none; ${ !is_activeRoutesTbas ? `display:none` : '' }`}>
                    <div class="ui-scrollview select-options len-6">
                        {#each activeRoutesTbas as odio}
                        <button on:click={()=>handleactiveRoutesTbasSelect(odio)} class={`select-option ${odio.active ? "active" : ""}`}>{odio.tabName}</button>
                        {/each}
                    </div>
                </div>
            </div>
            <div class="ui-select">
                <button on:click={handleactiveRoutesAssetsSelect} class="select-trigger">
                    <img src={RoutesAssets.img && RoutesAssets.img} class="coin-icon" style={` ${RoutesAssets.tabName !== "All Assets" ? "" : "display: none;" }`} alt="">
                    {RoutesAssets.tabName}
                    <div class="arrow active">
                        <Icon src={RiSystemArrowRightSLine}  size="18"  color="rgb(255, 255, 255)"   />
                    </div>
                </button>
                <div class="select-options-wrap"  style={`top: 100%; opacity: 1; transform: none; ${ !is_activeRoutesAssets ? `display:none` : '' }`}>
                    <div class="ui-scrollview select-options len-6">
                        {#each activeRoutesAccest as ksuey}
                        <button on:click={()=>handleactiveRoutesAssetsSelect(ksuey)}  class={`select-option ${ksuey.active ? "active" : ""}`}>
      ...
      <div class="select-trigger">Past 60 days<div class="arrow ">
         <Icon src={RiSystemArrowRightSLine}  size="18"  color="rgb(255, 255, 255)"   />
            ...
            <div class="ui-select">
                <button on:click={handleactiveRoutesRoutesDess} class="select-trigger">{RoutesDess.tabName}<div class="arrow ">
                    <Icon src={RiSystemArrowRightSLine}  size="18"  color="rgb(255, 255, 255)"   />
                </div>
                </button>
                <div class="select-options-wrap" style={`top: 100%; opacity: 1; transform: none; ${ !is_activeRoutesAs ? `display:none` : '' }`}>
                    <div class="ui-scrollview select-options len-6">
                        {#each activeRoutesAssets as ueys}
                        <button on:click={()=>handleactiveRoutesRoutesDess(ueys)}  class={`select-option ${ueys.active ? "active" : ""}`}>{ueys.tabName}</button>
                        {/each}
                    </div>
                </div>
            </div>
