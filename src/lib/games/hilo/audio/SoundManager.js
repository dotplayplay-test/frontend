        ...
        if (defaultSettings) {
            Object.keys(defaultSettings).forEach(setting => {
                if (this.audioMap[setting]) {
                    this.audioMap[setting] = { ...this.audioMap[setting], ...defaultSettings[setting] }
                }
            })
        }
        this.audioEnabled = Boolean(audioEnabled);

        Object.keys(this.audioMap).filter((key, _) => this.audioMap[key].enabled).forEach(key => this.play(key))
    }
