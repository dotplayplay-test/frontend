<script>
  ...
  import { liveStats, dice_troo } from "../store";
  import {
    Chart,
    CategoryScale,
    LinearScale,
  ...
  const { getStats, resetStats, recordGame } = useLiveStats(liveStats, "DICE_LIVE_STATS");
  ...
  onMount(() => {
    ...
    const unsub = dice_troo.subscribe(data => {
        recordGame(data.has_won, data.bet_amount, data.profit, data.token_img);
    });
    return () => {
      unsub();
