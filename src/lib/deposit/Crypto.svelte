<script>
  ...
  const handleFetchPendingOrder = async () => {
    ispo_loading = true;
    await axios
      .get(`${url}/api/deposit/pending-order`, {
        headers: {
  ...
  const handleSubmit = async () => {
    ...
    await axios
      .post(
        ...
        {
          data: {
            from_coin_id: from.coin.coin_name,
            to_coin_id: to.coin.coin_name,
            amount,
          },
        },
        {
  ...
  function handleCopyAmomut() {
    navigator.clipboard.writeText($deposit_info.amount).then(() => {
      amount_msg = "Copied";
      setTimeout(() => {
        amount_msg = "";
      }, 4000);
  ...
  function handleCopyCode() {
    navigator.clipboard.writeText($deposit_info.pay_address).then(() => {
      address_msg = "Copied";
      setTimeout(() => {
        address_msg = "";
      }, 4000);
      ...
      <div class="sbaeww">
        <div class="sq07zth page-margin">
          ...
          <div class={`ui-select select-tokens ${is_open ? "is-open" : ""} `}>
            <button
              disabled={$deposit_info}
              on:click={handleNetworks_isOpen}
              ...
              >
                <div class="ui-scrollview select-options len-9">
                  {#each networks as net (net.id)}
                    <button
                      class={`select-option ${
                        net.status === "active" ? "active" : ""
                      } `}
                      on:click={() => handleNetworks(net)}>{net.network}</button
                    >
                  {/each}
                </div>
              </div>
          ...
          {:else}
            <div class="sbz45td">
              <div class="qr-wrap">
                <div class="qz3bl7d">
                  <img
                    src="https://bc.game/api/game/support/qrcode/320/?text=0x8d3f78BDAd3F1eaa4e5B02E277D24a72C932505a"
                    alt="qr.png"
                  />
                </div>
              </div>
              <div class="address-info">
                ...
                {#if hours <= 0 && minutes <= 0 && minutes <= 0 && seconds <= 0}
                  <div>
                    Deposit Address has: <span
                      style="color: yellow; letter-spacing:1px"
                    >
                      Expired
                    </span>
                  </div>
                {:else}
                  <div>
                    Expire in: <span style="color: yellow; letter-spacing:1px">
                      {`${typeof hours === "number" && hours}hr : ${
                        typeof minutes === "number" && minutes
                      }m : ${typeof seconds === "number" && seconds}s`}</span
                    >
                  </div>
                {/if}
                ...
                <div class="cont">
                  ...
                  <button
                    ...
                    class="ui-button button-normal copy"
                  >
            ...
            <div class="d5oeih5">
              <p>
                <span class="cl-primary">NOTICE:</span> Send only USDT to this deposit
                address. Coins will be deposited automatically after 6 network confirmations.
