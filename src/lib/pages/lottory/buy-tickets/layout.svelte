<script>
	...
	import BiUpArrow from "svelte-icons-pack/bs/BsArrowUpShort";
	import BiDownArrow from "svelte-icons-pack/bs/BsArrowDownShort";
	...
	const handleSetTicketAmount = (amount, update) => {
		return (e) => {
			if (update)
				ticketAmount = Math.min(
					1000,
					Math.max(1, ticketAmount + amount),
				);
	...
	const buyTicket = async () => {
		loading = true;
		if ($default_Wallet.coin_name !== "PPL" || (isManual && (selectedNumbers.length < 5 || !jackpotNum)) || $default_Wallet.balance < ticketAmount) return;
		try {
			await axios.post(
	...
	>
		...
		>
			<div class="sc-dkPtRN jScFby scroll-view sc-bXdNzS jybDrO">
				<div class="warn">
					<span
						>You can get more NNL from <a href="/wallet/swap?to=BCL"
							>NGSwap</a
						> at anytime</span
					>
				</div>
				<div class="layout">
					...
					<div class="pay-input">
						...
						><span class="balance">{$default_Wallet.balance}</span>
					</div>
					<div
							...
							<div class="sc-kDTinF bswIvI button-group">
								...
								{#if showSlider}
									...
									>
										...
										<button
											on:click={handleSetTicketAmount(
												1000,
											)}
											class={ticketAmount === 1000
												? "active"
												: ""}>Max</button
										>
									</div>
						...
						<div class="suff-desc">
							<div class="cont">
								...
								<div class="amount error">
									{parseFloat(ticketAmount).toFixed(8)} PPL
								</div>
							</div>
						</div>
					</div>
					<div class="desc">1.00000000 PPL/Ticket</div>
				</div>
				<div class="sc-kjOQFR gwUFaO">
					...
					{#if isManual}
						...
						<div
							class="error-txt {selectedNumbers.length < 5 ||
							!jackpotNum
								? 'show'
								: ''}"
						>
				...
				<button
					...
					disabled={$default_Wallet.balance < ticketAmount ||
						(isManual &&
							(selectedNumbers.length < 5 || !jackpotNum))}
...
<style>
	...
	.gwUFaO .select-box .button.active {
		color: rgb(0, 0, 0);
		background: url(https://static.nanogames.io/assets/ball.161fa8af.png) 0%
			0% / 100% 100% no-repeat;
	}
