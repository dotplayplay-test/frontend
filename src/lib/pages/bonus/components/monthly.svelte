...
import { month_cashback} from "$lib/store/cashbacks"
...
const handleClaimcashbacks = (async()=>{
    is_loading = true
    await axios.post(`${url}/api/profile/claim-monthly-bonus`,
    { data: ""},
    {
        headers: {
    ...
    <div class="bonus-item-text">
        ...
        {#if $allcashback.vip_level > 21}
            <div class="bonus-desc">
                <p>Wager required</p>
                <p><span style="font-weight: bold; color:#fff">${$allcashback.monthly_cashback}</span> / $10,000</p>
            </div>
            <div class="bonus-desc last"><p>Ready to claim</p>
                <p>${($allcashback.monthly_cashback * 0.01 * 0.03).toFixed(6)}</p>
            </div>
            {:else}
            <div class="bonus-desc">
                <p>Wager required</p>
    ...
    <div class="bonus-btn">
        ...
        {#if $allcashback.month_bonus !== 0}
            ...
            <button  class="quests-btn">
                ...
                <span style="font-size: 16px;  font-family: 'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif; letter-spacing: 2px;">{$month_cashback}</span>
            </button>
        {/if}
    {/if}
